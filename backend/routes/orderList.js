...
const order = require('../models/orderLists')
...
// Get Single order
 router.get('/:id?', async(req,res)=> {
     const order_view = await order.findById({_id:req.params.id})
     res.json(order_view);
 })
 // Delete order
 router.delete('/deleteAddress/:index', async(req,res)=> {
    ...
    console.log(index)
    const order_obj = await order.findById({_id: 1})
    order_obj.order_address.splice(index, 1)
    const order_delete = new order(order_obj)
...
router.post('/create', async(req,res)=> {
    ...
    const order_create = new order(order_obj)
...
router.put('/updateAddress/:index', async(req,res)=> {
    const index = req.params.index
    const order_update = await order.findById({_id:1})
    order_update.order_address[index] = req.body
    const order_create = new order(order_update)
    const savedorder = await order_create.save()
    res.json(savedorder);
